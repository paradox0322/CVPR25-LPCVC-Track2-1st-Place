...
class XDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # {'bbox': False, 'mask': True, 'caption': True, 'captioning': False, 'retrieval': False, 'grounding': True}
    ...
    def forward(self, x, mask_features, mask=None, target_queries=None, target_vlp=None, task='seg', extra={}):
        ...
        for i in range(self.num_layers):
            ...
            # re-implement into QNN-allowed format
            # change mask value from bool type to float type
            # attn_mask[torch.where(attn_mask.sum(-1) == attn_mask.shape[-1])] = False
            attn_mask[torch.where(attn_mask.sum(-1) == attn_mask.shape[-1])] = False
            attn_mask = (attn_mask.float() - 1) * (10000)
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size, layer_id=-1, task='seg'):
        ...
        if self.task_switch['mask']:
            ...
            _, q, _ = mask_embed.shape
            outputs_mask = torch.matmul(mask_embed, mask_features.view(b, c, -1))
            outputs_mask = outputs_mask.view(b, q, h, w)
            ...
            attn_mask = F.interpolate(outputs_mask, size=attn_mask_target_size, mode="bilinear", align_corners=False, antialias=False)

            # must use bool type
            # If a BoolTensor is provided, positions with ``True`` are not allowed to attend while ``False`` values will be unchanged.
            #re-implement into QNN-allowed format
            # attn_mask = (attn_mask.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1) < 0.5).bool()
            attn_mask = (attn_mask.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1) < 0.5).float()

            attn_mask = attn_mask.detach()

            # NOTE: fill False for cls token (JY)
            # attn_mask[:, self.num_queries:self.num_queries+1].fill_(False)
            attn_mask[:, self.num_queries:self.num_queries+1].fill_(0.0)
        else:
            outputs_mask = None
